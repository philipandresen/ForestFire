function [ outstring ] = NAPALM_parallel_script()
...
prompt={'First frame';'Last frame';'Background noise (photons)';...
    ...
    'Number of independent threads:'; 'Number of fitting iterations';...
    'Approximate background subtraction?' ;'Input the previous output?'};
name='NAPALM Parallel initialization';
...
formats(11,1)=struct('type','edit','format','integer','limits',[1 8],'style','edit');
formats(12,1)=struct('type','edit','format','integer','limits',[1 100],'style','edit');
formats(13,1)=struct('type','edit','format','integer','limits',[0 8],'style','edit');
formats(14,1)=struct('type','check','format','integer','limits',[0 1],'style','checkbox');
defaultanswer={1;10000;4.85;0.136;'[20]';40;3;2;6;cd;7;100;0;1};

[inputs,canceled]=inputsdlg(prompt,name,formats,defaultanswer);
if canceled==1; outstring=[]; return; end;
file='original_files';
if inputs{14}==1; file='output_files'; end;
outstring={['threshold= ' inputs{5} ';'];...
    ['output_files = NAPALM_parallel(' ...
    ...
    num2str(inputs{11}) ',' num2str(inputs{12}) ',' num2str(inputs{13}) ...
    ',' '''' inputs{10} '''' ',' file ');']};
